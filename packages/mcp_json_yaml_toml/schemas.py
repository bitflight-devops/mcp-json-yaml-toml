...
def _load_default_ide_patterns() -> list[str]:
    ...
    try:
        ...
        if default_stores_path.exists():
            data = orjson.loads(default_stores_path.read_bytes())
            return data.get("ide_patterns", [])
    except Exception:
        pass
    return []
...
class SchemaManager:
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if self.config_path.exists():
            try:
                return orjson.loads(self.config_path.read_bytes())
            except orjson.JSONDecodeError:
                pass
    ...
    def get_schema_for_file(self, file_path: Path) -> dict[str, Any] | None:
        ...
        for schema_info in catalog.get("schemas", []):
            if "fileMatch" in schema_info:
                if filename in schema_info["fileMatch"]:
    ...
    def _get_catalog(self) -> dict[str, Any] | None:
        ...
        if self._is_cache_valid(self.catalog_path):
            try:
                return orjson.loads(self.catalog_path.read_bytes())
            except orjson.JSONDecodeError:
    ...
    def scan_for_schema_dirs(self, search_paths: list[Path], max_depth: int = 5) -> list[Path]:
        ...
        self.config["last_scan"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
        self._save_config()
