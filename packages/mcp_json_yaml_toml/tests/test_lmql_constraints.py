...
class TestValidationResult:
    ...
    def test_validation_result_when_valid_then_no_error(self) -> None:
        result = ValidationResult(valid=True)
        assert result.valid is True
        assert result.error is None
        assert result.is_partial is False

    def test_validation_result_when_invalid_then_has_error(self) -> None:
        result = ValidationResult(valid=False, error="Invalid input")
        assert result.valid is False
        assert result.error == "Invalid input"

    def test_validation_result_when_partial_then_has_remaining(self) -> None:
    ...
    def test_validation_result_when_minimal_then_dict_minimal(self) -> None:
        result = ValidationResult(valid=True)
        d = result.to_dict()
        assert d == {"valid": True}

    def test_validation_result_when_full_then_dict_complete(self) -> None:
        result = ValidationResult(
            valid=False,
            error="test error",
            is_partial=True,
...
class TestYQPathConstraint:
    ...
    def test_yq_path_when_valid_then_accepts(self, path: str) -> None:
        result = YQPathConstraint.validate(path)
        assert result.valid is True

    def test_yq_path_when_empty_then_partial(self) -> None:
        result = YQPathConstraint.validate("")
        assert result.valid is False
        assert result.is_partial is True

    def test_yq_path_when_missing_dot_then_suggests(self) -> None:
        result = YQPathConstraint.validate("users")
        assert result.valid is False
        assert ".users" in result.suggestions

    def test_yq_path_when_partial_then_evaluates(self) -> None:
    ...
    def test_yq_path_when_get_definition_then_returns_expected(self) -> None:
        """
...
class TestYQExpressionConstraint:
    ...
    def test_yq_expression_when_valid_then_accepts(self, expression: str) -> None:
        result = YQExpressionConstraint.validate(expression)
        assert result.valid is True

    def test_yq_expression_when_empty_then_partial(self) -> None:
        result = YQExpressionConstraint.validate("")
        assert result.valid is False
        assert result.is_partial is True

    def test_yq_expression_when_missing_dot_then_rejects(self) -> None:
        """
...
class TestConfigFormatConstraint:
    ...
    def test_config_format_when_valid_then_accepts(self, fmt: str) -> None:
        result = ConfigFormatConstraint.validate(fmt)
        assert result.valid is True

    def test_config_format_when_invalid_then_suggests(self) -> None:
        result = ConfigFormatConstraint.validate("csv")
        assert result.valid is False
        assert "json" in result.suggestions

    def test_config_format_when_partial_match_then_suggests(self) -> None:
        result = ConfigFormatConstraint.validate("js")
        assert result.valid is False
        assert result.is_partial is True
        assert "json" in result.suggestions

    def test_config_format_when_empty_then_partial(self) -> None:
        result = ConfigFormatConstraint.validate("")
        assert result.valid is False
        assert result.is_partial is True

    def test_config_format_when_get_definition_then_has_allowed_values(self) -> None:
...
class TestIntConstraint:
    ...
    def test_int_constraint_when_valid_then_accepts(self, value: str) -> None:
        result = IntConstraint.validate(value)
        assert result.valid is True

    @pytest.mark.parametrize("value", ["3.14", "abc"])
    def test_int_constraint_when_invalid_then_rejects(self, value: str) -> None:
        result = IntConstraint.validate(value)
        assert result.valid is False

    def test_int_constraint_when_letters_mixed_then_reports_invalid_char(self) -> None:
        result = IntConstraint.validate("12a")
        assert result.valid is False
        assert result.error is not None
        assert "Invalid character" in result.error

    def test_int_constraint_when_empty_then_partial(self) -> None:
        result = IntConstraint.validate("")
        assert result.valid is False
        assert result.is_partial is True

    def test_int_constraint_when_lone_minus_then_partial(self) -> None:
...
class TestKeyPathConstraint:
    ...
    def test_key_path_when_valid_then_accepts(self, key: str) -> None:
        result = KeyPathConstraint.validate(key)
        assert result.valid is True

    def test_key_path_when_empty_then_partial(self) -> None:
...
class TestJSONValueConstraint:
    ...
    def test_json_value_when_valid_then_accepts(self, value: str) -> None:
        result = JSONValueConstraint.validate(value)
        assert result.valid is True

    def test_json_value_when_incomplete_string_then_partial(self) -> None:
    ...
    def test_json_value_when_incomplete_array_then_partial(self) -> None:
    ...
    def test_json_value_when_incomplete_object_then_partial(self) -> None:
    ...
    def test_json_value_when_empty_then_partial(self) -> None:
...
class TestFilePathConstraint:
    ...
    def test_file_path_when_valid_then_accepts(self, path: str) -> None:
        result = FilePathConstraint.validate(path)
        assert result.valid is True

    def test_file_path_when_empty_then_rejects(self) -> None:
        result = FilePathConstraint.validate("")
        assert result.valid is False

    def test_file_path_when_invalid_chars_then_rejects(self) -> None:
        result = FilePathConstraint.validate("config<test>.json")
        assert result.valid is False
        assert result.error is not None
        assert "Invalid characters" in result.error

    def test_file_path_when_null_byte_then_rejects(self) -> None:
...
class TestConstraintRegistry:
    ...
    def test_registry_when_known_constraint_then_returns(self) -> None:
        constraint = ConstraintRegistry.get("YQ_PATH")
        assert constraint is YQPathConstraint

    def test_registry_when_unknown_constraint_then_returns_none(self) -> None:
        constraint = ConstraintRegistry.get("UNKNOWN")
        assert constraint is None

    def test_registry_when_validate_known_then_validates(self) -> None:
        result = ConstraintRegistry.validate("YQ_PATH", ".name")
        assert result.valid is True

    def test_registry_when_validate_unknown_then_errors(self) -> None:
        result = ConstraintRegistry.validate("UNKNOWN", "value")
        assert result.valid is False
        assert result.error is not None
        assert "Unknown constraint" in result.error

    def test_registry_when_list_then_includes_expected(self) -> None:
    ...
    def test_registry_when_get_all_definitions_then_complete(self) -> None:
...
class TestDynamicConstraints:
    ...
    def test_create_enum_constraint_when_valid_then_validates(self) -> None:
    ...
    def test_create_pattern_constraint_when_valid_then_validates(self) -> None:
...
class TestValidationHelpers:
    ...
    def test_validate_tool_input_when_valid_then_passes(self) -> None:
        result = validate_tool_input("YQ_PATH", ".name")
        assert result.valid is True

    def test_validate_tool_input_when_invalid_then_fails(self) -> None:
        result = validate_tool_input("YQ_PATH", "invalid")
        assert result.valid is False

    def test_validate_tool_input_when_raise_flag_then_raises(self) -> None:
        from fastmcp.exceptions import ToolError

        with pytest.raises(ToolError):
            validate_tool_input("YQ_PATH", "invalid", raise_on_invalid=True)

    def test_get_constraint_hint_when_valid_then_returns_none(self) -> None:
        hint = get_constraint_hint("YQ_PATH", ".name")
        assert hint is None

    def test_get_constraint_hint_when_invalid_then_returns_hint(self) -> None:
...
class TestLMQLRegexIntegration:
    ...
    def test_regex_when_fullmatch_then_evaluates(self) -> None:
    ...
    def test_regex_when_is_prefix_then_checks_partial(self) -> None:
    ...
    def test_regex_when_derivative_then_computes(self) -> None:
