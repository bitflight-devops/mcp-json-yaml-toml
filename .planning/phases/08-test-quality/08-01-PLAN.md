...
must_haves:
  truths:
    ...
    - "test_diff.py, test_lmql_constraints.py, and test_yq_wrapper.py have fewer duplicate test methods after consolidation into parametrized tests"
  artifacts:
  ...
  key_links:
    - from: "packages/mcp_json_yaml_toml/tests/test_schemas.py"
      to: "packages/mcp_json_yaml_toml/schemas/ide_cache.py"
      via: "_build_ide_schema_index public API (calls _parse_extension_schemas internally)"
      pattern: "_build_ide_schema_index"
---
...
Refactor ALL four tests to use `_build_ide_schema_index()` as the entry point. `_build_ide_schema_index()` calls `_parse_extension_schemas` internally, so the same behavior is exercised through the real integration path. Do NOT call `_parse_extension_schemas` directly in any test.
</action>
<verify>
...
# Verify test_hints actually runs and passes (not just exists)
uv run pytest packages/mcp_json_yaml_toml/tests/verify_features.py::test_hints -v 2>&1 | tail -5
```

All tests pass. `verify_features.py::test_hints` appears as PASSED in verbose output (confirming the assertions execute, not just exist). test_schemas.py no longer has direct `_parse_extension_schemas` calls (uses `_build_ide_schema_index` instead).
</verify>
<done>
